...
(defn ident-value [{:keys [ast]}]
  (if (vector? (:key ast)) (second (:key ast))))

(defn ensure-attrs [{:keys [::entity parser] :as env} attributes]
  (merge entity (parser env (filterv (-> entity keys set complement) attributes))))
